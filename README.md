...
- **Organize with projects** — create projects (and nested subprojects) and link any task to one or more projects. Each project automatically tracks its total time, including time from subprojects. Submitting the project form with an empty name shows an inline "Project name is required." error.
...
- **Weekly overview** — the Overview page shows all seven days of the selected week in a grid, with per-day totals, a week total, and prev/next navigation to browse past or future weeks. Clicking any task entry in a day column navigates directly to the Tasks page pre-filtered to show only that day's tasks.
- **Monthly overview** — the Overview page's Month tab shows a full calendar grid of the selected month. Each day cell displays its tracked total; clicking a day opens a panel listing that day's tasks with durations. Clicking a task in the panel navigates to the Tasks page filtered to that specific day. Prev/next navigation and a monthly total are included.
...
- **Search and filter tasks** — a filter panel above the task list lets you search by description keyword (debounced 300 ms), filter by project (includes all subprojects), and filter by date range using clearly labelled "Start date" and "End date" inputs. Filters combine with AND logic. A Reset button clears all filters at once.
...
- **Task templates** — users can save frequently-used task configurations as templates (name, optional description, optional project associations). A "Task Templates" section on the Dashboard shows all personal templates as cards with a name, description preview, and project chips. Clicking **▶ Start** on a template creates a running task pre-filled with the template's description (or the template name when description is blank) and project associations; the timer starts from the total time already accumulated across all previous completed sessions for that description, so repeated work on the same activity shows a continuously growing counter rather than resetting to zero; the topbar stop button and the template card's "● Running" indicator appear immediately without any page refresh. Templates can be created via a "New Template" form (with project multi-select), edited inline (pencil button), and deleted (with a confirmation dialog). Submitting the form with an empty name shows an inline "Template name is required." error. While any timer is running, Start buttons on all other template cards are disabled. Templates are strictly private — users can only access their own. `POST /api/task-templates/{id}/start` returns 409 if a timer is already running. All CRUD operations are available via `GET/POST /api/task-templates` and `PUT/DELETE /api/task-templates/{id}`.
- **Add task from project page** — a "+ Add Task" button on each project detail page navigates directly to the Tasks page with that project pre-selected in the filter dropdown and the create form open, saving the extra clicks of manually setting the project filter after opening the form.
- **Productivity analytics** — a dedicated "Analytics" page (accessible from the top navigation) shows two visualisations derived from the user's tracked history. The **Activity Heatmap** renders a GitHub-style 52–53-week grid (7 rows × N columns, Monday-first). Each cell represents one calendar day and is colour-coded in four levels based on the day's share of the year's maximum: grey (#ebedf0) for zero, light green (#9be9a8) for < 25 %, medium green (#40c463) for 25–50 %, dark green (#30a14e) for 50–75 %, and darkest green (#216e39) at the top tier. Hovering a cell shows a tooltip with the date and formatted duration. A year selector at the top-right lets users browse any year (current year +5 / −10, giving 16 options). Day boundaries are calculated in the user's preferred timezone (integrated with US-025). The **Average Hours by Day of Week** bar chart (labelled "avg per weekday, last 12 weeks") shows the average tracked seconds per weekday (MON–SUN) over the last 12 weeks — each bar represents a day of the week, not an individual week, and shows average time across the 12-week window. Bar heights are proportional to the day with the most average time; each bar shows the formatted average below it. Only completed tasks (tasks with both `startTime` and `endTime`) are included in both visualisations; running timers are excluded. Data is always scoped to the authenticated user. API: `GET /api/analytics/heatmap?year=<int>` and `GET /api/analytics/weekly-pattern?weeks=<int>` (defaults: current year, 12 weeks).
- **Shared project breakdown** — the Analytics page includes a **Shared Project Breakdown** section that shows, for each project where more than one member has tracked time, a ranked list of contributors with their share of total tracked time as a percentage bar. The breakdown uses a 12-week rolling window aligned to the selected year: `to = min(now, yearEnd)`, `from = to − 12 × 7 days` (future years return zero results because `from > to`). Projects with only a single contributor are excluded. API: `GET /api/analytics/shared-breakdown?weeks=12&year=<int>`.
...
| `TaskServiceTest` | 41 | All task service operations including project association, date filtering, keyword search, projectId filter (unit); ArgumentCaptor tests kill setter/boundary/lambda mutations; accumulated-seconds field set on start, default zero for regular start |
| `ProjectServiceTest` | 24 | All project service operations including member management (unit) |
| `TaskTemplateServiceTest` | 17 | Create/list/update/delete template (unit), startFromTemplate with description+projects, null description falls back to template name, template-not-found, unknown project, user-not-found, accumulated previous session seconds |
...
| `TaskTemplateTest` | 12 | US-027 Task Templates: create with all fields, no-description/no-project create, list returns only own templates, update name+desc+projects, delete returns 204, start-from-template creates running task with description+projects, start while running 409, cross-user isolation (PUT/DELETE/start on other's template → 404), multi-project template in list, 401 without auth, 400 blank name, accumulated time from previous sessions on second start |
| `AnalyticsControllerTest` | 17 | US-028 Productivity Analytics: heatmap correct day + total seconds, multiple tasks same day aggregated, empty year returns empty days list, running timer excluded from heatmap, heatmap data scoped to authenticated user, heatmap 401 without auth, weekly pattern returns 7 entries MON–SUN, weekly pattern computes correct average for recent task, weekly pattern 401 without auth, weekly pattern data scoped to authenticated user, future-year query returns all zeros, past-year query excludes current-year tasks, shared-breakdown 401 without auth, no shared projects returns empty list, shared project includes contributions for both members with correct percentages, project with no activity excluded, shared-breakdown data scoped to authenticated user |
...
| `us028-productivity-analytics.spec.ts` | US-028 | Heatmap returns today's entry, weekly-pattern 7 entries MON–SUN, both 401 without JWT, data scoped to user, heatmap ≥ 364 cells, 7 day bars, year selector updates without reload, empty user sees empty heatmap |
...
| `DashboardPage.test.jsx` | 31 | Timer start/stop, active task display, summary cards (today/week), top projects list, running task info, refresh after timer actions; template list renders, Start button calls API + refreshes topbar immediately (no reload), running indicator shown when active task matches template name, Start disabled when other timer running, New Template form, create submit, edit form pre-fill, save edit, delete confirm dialog, project checkboxes in form |
...
| `TasksPage.test.jsx` | 38 | Create, edit, delete tasks; project multi-select on create/edit; project display in task row; field-level error extraction from 400 responses; Add Task button reachable in 1 click; filter panel rendered, search debounce, project filter, date range, no-results message, reset; URL ?projectId= seeds filter dropdown; URL ?projectId=&addTask=true auto-opens form with project pre-checked |
...
| `ProjectDetailPage.test.jsx` | 51 | Date-range presets, custom range form, project name/desc/total, subproject totals, task list, running task, error states, back navigation; members section rendered; member list shows name+email+role; invite form visible to owner only; invite API called with correct email; invite error shown (clears when user types new email); remove button only for MEMBER rows; removeMember API called; member count in header; contributors card for shared projects; no contributors card for solo; user-filter dropdown shown; dropdown change re-fetches with userId; task owner name on shared rows; no owner name for solo; Add Task button links to /tasks with project pre-selected; Export button renders; modal opens/closes; format radios (CSV/JSON); scope radios (All Time/Specific Month); year+month inputs appear for month scope; Download calls exportProject API and triggers blob download; JSON format passes correct param; month scope passes year+month params; error shown when export fails |
| `Layout.test.jsx` | 14 | Topbar timer visible/hidden, elapsed from startTime, timer on all pages, API called once on mount |
| `AnalyticsPage.test.jsx` | 20 | Analytics page renders, heatmap grid present, heatmap cells for days with data, year selector shows current year, year change triggers re-fetch of heatmap, year change triggers re-fetch of weekly pattern with same year, weekly pattern subtitle shows selected year, weekly pattern chart renders, 7 day-of-week bars MON–SUN, non-zero bar height for active days, heatmap legend rendered, correct cell color for high activity, tooltip title attribute on cells, heatmap grid correct total cell count, empty-state message when no heatmap days, error message when heatmap fetch fails, breakdown section rendered, empty state when no shared projects, shared project contributions rendered, re-fetches breakdown when year changes |
...
| GET | `/api/analytics/shared-breakdown` | `?weeks=<int>` (default: 12), `?year=<int>` (default: current year) | 200 `SharedBreakdownResponse` | Returns one entry per shared project where ≥ 2 members tracked time in the window; projects with only one contributor or zero tracked time are excluded; contributions sorted descending by totalSeconds |
...
Shared breakdown response shape:
```json
{
  "projects": [
    {
      ...
      "projectName": "Thesis",
      "contributions": [
        { "userId": 1, "userName": "Alice", "totalSeconds": 7200, "percentage": 66.67 },
        { "userId": 2, "userName": "Bob",   "totalSeconds": 3600, "percentage": 33.33 }
      ]
    }
  ]
}
...
| PUT | `/api/projects/{id}` | `{name, description?, budgetHours?, parentProjectId?}` | 200 | Rename / re-describe / set budget / reparent (OWNER only); null `budgetHours` clears budget; null `parentProjectId` makes project root-level; circular parent → 400 |
...
- **Project sharing membership model** — a separate `project_members` table stores `(project_id, user_id, role, joined_at)` with a unique constraint on `(project_id, user_id)`. `project.user_id` is kept as the original owner FK for backward compatibility. Access checks use the membership table: `findByIdAndMember` for read operations (any member), `findByIdAndUser` for write operations (owner only). `MembershipSeeder` runs on startup to back-fill OWNER rows for all projects created before this feature was added. `UserNotFoundException` (plain `RuntimeException`) is used instead of Spring Security's `UsernameNotFoundException` when the invitee email is not registered, to prevent the exception from being intercepted by Spring Security's exception handling as a 401.
- **Timezone display without a library** — all timezone conversion uses the built-in `Intl.DateTimeFormat` API (`Intl.DateTimeFormat`, `formatToParts`, `en-CA` locale for deterministic `YYYY-MM-DD` output). `localDateToUtcIso` converts a "local" datetime-local input value to UTC using a naive-UTC + offset-measurement approach: treat the input as UTC, measure the offset the target timezone shows for that UTC instant, apply the correction. One iteration is accurate for all standard and DST zones. The `timezone` column on the `User` entity defaults to `"UTC"` so pre-existing accounts work without migration. IANA timezone IDs are validated at the Spring layer using `ZoneId.of()`, which throws `DateTimeException` on unrecognised strings; `GlobalExceptionHandler` catches `InvalidTimezoneException` and returns HTTP 400.
- **Task templates** — `TaskTemplate` is a separate JPA entity (`task_templates` table) owned by a `User` with a `@ManyToMany` to `Project` via a `template_projects` join table. `startFromTemplate` delegates to `TaskService.startTask(userEmail, request, projectIds)` — an overloaded method added to `TaskService` so that `startTask` called from `TaskController` (no projects) and from templates (with projects) share the same 409-if-running logic without duplication. Ownership is enforced via `findByIdAndUser` — callers from other users get a 404 (not a 403) to avoid leaking template existence. Project associations on a template are resolved with `findByIdAndMember` so users can only link projects they are members of (preventing association with arbitrary project IDs). Deleting a template clears its project join-table entries before the delete to avoid FK constraint violations.
- **Project time budgets** — `budgetHours` is a nullable `Double` column on the `Project` entity. `ProjectService.computeBudgetStatus(budgetHours, usedHours)` is a `static` helper so both `ProjectService.getProjectSummary` and `DashboardService` can compute budget status without a circular dependency. Budget percentage is capped at 100% for the visual progress bar (the label still shows the true percentage). The three status values (`ON_TRACK`, `WARNING`, `OVER_BUDGET`) are thresholded at 80% and 100% and used by the frontend to select the bar colour and optional "Over budget" badge. Budget usage is always an all-time aggregate of the full project subtree across all members (i.e. it is independent of the date-range filter on the summary).
- **Per-user contribution breakdown** — `ProjectSummaryResponse` now carries a `contributions` list (per-user totals) and `userId`/`userName` on every `TaskSummary` entry. `getProjectSummary` collects all task entities from the subtree via `collectSubtreeTaskEntities` (deduplication by task id), groups them by `Task.user` for contributions, and optionally filters `tasks` + `totalSeconds` when `?userId=` is present. The `contributions` array is always the full per-user breakdown regardless of the user filter, so the frontend dropdown remains functional. `AccessDeniedException` (403) is thrown when the `userId` param belongs to a non-member. The same `userId` filter on `GET /api/tasks` validates that both the caller and the target user are project members using `projectRepository.findByIdAndMember(projectId, targetUser)` — no new repository dependency needed in `TaskService`.
- **Productivity analytics** — `AnalyticsService` uses `findByUserAndStartTimeBetweenOrderByStartTimeAsc` (the same repository method used by daily/weekly views) for both heatmap and weekly-pattern queries, reusing the existing index on `(user_id, start_time)` with no new indexes needed. Heatmap year boundaries are computed as `ZonedDateTime.of(year, 1, 1, 0, 0, 0, 0, zone).toInstant()` using the user's IANA timezone, so users in e.g. `America/New_York` see the correct day split. Running tasks (endTime = null) and zero-second tasks are excluded. The weekly-pattern range is a rolling window: `Instant.now().minus(weeks * 7, ChronoUnit.DAYS)` — it always covers the most recent N full weeks regardless of the current day. Average is computed as `total / weeks` (not `total / distinct-days-with-data`), giving a true per-week average. Frontend heatmap: `buildHeatmapCells` creates a flat cell array padded to complete weeks using `(Jan1.getDay() + 6) % 7` (converts JS Sunday=0 to Monday=0), then rendered via a CSS `grid-template-rows: repeat(7, 14px); grid-auto-flow: column` grid — this naturally produces week columns from a flat row-major array.
- **`UpdateProjectRequest` includes `parentProjectId`** — `PUT /api/projects/{id}` now accepts `parentProjectId` in its request body. `ProjectService.updateProject` calls `guardAgainstCircularHierarchy` when a new parent is specified, returning HTTP 400 (`CircularProjectHierarchyException`) if the target parent is a descendant of the project being updated. Passing `null` (or omitting the field) clears any existing parent, making the project a root-level project. This unifies the circular-hierarchy guard between create and update paths.
- **401 interceptor in `authApi.js`** — an Axios response interceptor automatically clears `tt_token` / `tt_user` from `localStorage` and redirects to `/login` when any non-auth API call returns HTTP 401. Auth-endpoint 401s (wrong password, expired credentials on `/auth/*`) are excluded from this redirect so that the login form can display the error message normally. This replaces the purely client-side token truthy-check in `ProtectedRoute`, ensuring that an invalid or expired JWT stored in `localStorage` causes an immediate redirect on the first protected API call.
- **JS-only form validation in `LoginPage`** — HTML5 `required` attributes were removed from all login and register inputs, replaced with explicit client-side checks in `handleSubmit`. This allows Playwright E2E tests to submit the empty form and observe React-rendered field-level error elements (`data-testid="error-email"` etc.), which were previously blocked by the browser's native validation tooltip mechanism before the React event handler could run.
- **E2E timer-stop duration guard (US-006)** — `us006-stop-timer.spec.ts` adds `waitForTimeout(1_200)` between `start-btn` and `stop-btn` clicks so the stopped task always accumulates at least one second of duration. Without this the task ends in ~0 ms, `todaySeconds` stays 0, and the `today-seconds != "00:00:00"` assertion fails.
- **Date filter inputs use `type="text"` (US-019)** — `filter-from` and `filter-to` inputs in `TasksPage.jsx` were changed from `type="date"` to `type="text"` (with `placeholder="YYYY-MM-DD"`). Chromium's native date picker does not reliably fire React's synthetic `onChange` when the value is set programmatically (via Playwright's `fill()` or `evaluate`), causing E2E tests to fail because `filterFrom` was never updated and the date filter was never applied. Plain text inputs work correctly with Playwright's `fill()`. The `fetchTasks` function already parses the value with `new Date(filterFrom).toISOString()` which accepts `YYYY-MM-DD` strings from either input type.
- **Self-contained reset test (US-019)** — the "reset button restores full task list" test now creates its own task via the API rather than depending on tasks left by earlier tests. A `waitForResponse` promise is registered before clicking the reset button and awaited immediately after, removing the reliance on a fixed 500 ms timeout for the re-fetch to complete.
- **US-005 DOM-detach fix** — after stopping a running timer in `beforeEach`, the spec now calls `page.reload()` before proceeding. Stopping the timer triggers rapid re-renders (`setActiveTask(null)` followed by `setElapsed('00:00:00')`) that briefly unmount `start-btn` from the DOM; Playwright's `click()` keeps retrying until the 30-second timeout. Reloading gives React a clean mount with no active timer, so `start-btn` is stable from the start of the test.
- **Playwright parallel workers** — `playwright.config.ts` increased `workers` from 1 to 4 in CI. Each spec file creates a unique user (`us005${Date.now()}@e2e.test`, etc.) so parallel files operate on isolated data with no conflicts. Expected CI time reduction: ~2.5 min → ~45 s.
- **US-019 DOM-update race fix** — replaced `waitForResponse` + immediate `rows.count()` with `expect(rows).not.toHaveCount(allCount, { timeout: 8_000 })`. The `waitForResponse` event fires at the network layer before the browser's JS `fetch().then()` callback runs and React re-renders; checking `rows.count()` immediately after `waitForResponse` reads the pre-update DOM and returns the old count. The Playwright assertion polls the locator count until it changes, ensuring the assertion runs after the DOM reflects the filtered results.
- **Analytics weekly pattern synced to selected year** — `GET /api/analytics/weekly-pattern` now accepts an optional `?year=` parameter. The upper time bound `to` is computed as `min(Instant.now(), end-of-year)` — current year ends today, past years end at `Jan 1 of year+1` in the user's timezone, and future years also return `to = Instant.now()` but with `from = yearStart` (which is in the future), so `from > to` and the repository query returns zero rows, producing all-zero averages. This ensures that future years never leak current-year data. The frontend passes `selectedYear` to `getWeeklyPattern` on mount and on every year-selector change. The subtitle now reads "avg per weekday, last 12 weeks of {year}" to make clear that (a) it is an average divided by 12, not a daily total, and (b) it reflects the year currently shown in the heatmap.
- **Dashboard timezone boundary fix** — `DashboardService.getSummary` previously hardcoded `ZoneOffset.UTC` for all day/week boundary calculations. Today-start, today-end, week-start, and week-end are now computed using `ZoneId.of(user.getTimezone())` and `ZonedDateTime.toLocalDate().atStartOfDay(zone)`. DST-safe date arithmetic uses `plusDays(1)` and `plusWeeks(1)` on `ZonedDateTime` instead of adding raw seconds to `Instant`. This makes the Dashboard "This Week" total consistent with the Overview page "Week Total" for users in any timezone.
- **URL param navigation from project detail to task form** — `TasksPage` reads `?projectId=` and `?addTask=true` from `useSearchParams` on mount. `?projectId=` seeds `filterProjectId` (the filter dropdown) and pre-checks that project's checkbox in the create form. `?addTask=true` auto-opens the form and sets `startTime`/`endTime` to the current time. This enables one-click task creation from a project context without losing the project association.
- **US-019 AC3 date-filter via URL params + backend from-only fix** — Two bugs combined to cause the failure. (1) `TaskService.listTasks` only activated the date filter when **both** `from` and `to` were non-null; a `from`-only request silently fell to the else branch and returned all tasks. Fixed by adding two new repository queries (`>= from`, `<= to`) and routing through them when only one bound is provided. (2) `TasksPage.jsx` was changed to seed `searchKw`, `debouncedSearch`, `filterFrom`, and `filterTo` from URL search params on mount (via `useSearchParams`), and the AC3 E2E test was changed to navigate directly to `/tasks?search=GAMMA&from=${todayStr}` (navigating via `/dashboard` first so TasksPage fully unmounts and remounts with the URL params applied). Four backend integration tests (`fromOnly`, `toOnly`, `searchAndFromOnly`, existing `searchAndBoth`) were added to cover the new code paths.
- **US-005 beforeEach API-stop fix** — the `page.reload()` approach introduced after the UI stop caused `start-btn` to be missing post-reload (the page likely redirected to `/login` via the 401 interceptor or showed a loading state that didn't resolve within 8 s). Fixed by calling `POST /api/tasks/stop` via `request.newContext()` in `beforeEach` **before** navigating to `/dashboard`. The timer is stopped at the database level before the page even loads, so `getActiveTask()` returns 404 on mount and `start-btn` is stable from the first render with no reload needed.
- **Accumulated timer for task templates** — `Task` has a `totalPreviousSeconds` column (bigint, default 0). When `TaskTemplateService.startFromTemplate` is called, it queries all completed tasks (`endTime IS NOT NULL`) for that user with the same description as the template, sums their durations (excluding zero-second entries), and passes the total to `TaskService.startTask` as the fourth argument. The existing 3-arg `startTask` overload delegates to the 4-arg version with `0L` for backward compatibility. The frontend `TimerContext` computes elapsed display as `totalPreviousSeconds + (now − startTime)`, so the topbar timer shows the true accumulated total across all sessions. This avoids creating a "resume" concept at the entity level — each session is still a separate `Task` row, which keeps the task list accurate; the accumulated total is a UI-only rollup computed fresh on each `startFromTemplate` call.
- **Shared project breakdown** — `GET /api/analytics/shared-breakdown?weeks=12&year=<int>` returns per-contributor time breakdowns for shared projects. The time window is computed as `to = min(Instant.now(), yearEnd)` and `from = to - weeks * 7 days`. For future years, `yearStart > Instant.now()` means `from = yearStart > to`, so the repository query returns zero rows and the project is excluded. Projects with fewer than two distinct contributors (or zero total seconds in the window) are excluded from the response. Contributions are sorted descending by `totalSeconds`; percentage is `(userSeconds / projectTotal) * 100` rounded to two decimal places. The query reuses `ProjectMemberRepository` to find projects where the caller is a member, then `TaskRepository.findByUserAndProjectAndStartTimeBetween` per member — no new DB indexes are needed because the existing `(user_id, start_time)` index covers the query. Frontend renders a percentage bar per contributor using an inline `width` style derived from the `percentage` field.
