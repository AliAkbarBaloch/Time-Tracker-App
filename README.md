...
- **US-019 AC3 date-filter via URL params** — Playwright's `fill()` on the controlled `filter-from` text input was consistently failing to trigger React's synthetic `onChange` in CI (count remained at `allCount` for the full 8-second polling window across both tries). Fixed by: (1) adding `useSearchParams` from `react-router-dom` to `TasksPage.jsx` so that `searchKw`, `debouncedSearch`, `filterFrom`, and `filterTo` are seeded from URL query params on mount; (2) changing the AC3 test to navigate directly to `/tasks?search=GAMMA&from=${todayStr}` — no UI input interaction needed. The combined filter is applied immediately by `fetchTasks` on component mount. `waitForLoadState('networkidle')` replaces the unreliable polling assertion.
