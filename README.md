...
- **US-005 DOM-detach fix** — after stopping a running timer in `beforeEach`, the spec now calls `waitForLoadState('networkidle')` instead of a fixed `waitForTimeout(400)`. The stop action triggers two sequential API calls (`stopTask` + `fetchSummary`), and Chromium re-renders the dashboard component after each response. `networkidle` waits until all outstanding requests have settled (500 ms of silence), ensuring `start-btn` is stable before the test body tries to click it.
