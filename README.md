...
- **Analytics weekly pattern synced to selected year** — `GET /api/analytics/weekly-pattern` now accepts an optional `?year=` parameter. When the selected year equals or exceeds the current year, the 12-week window ends at `Instant.now()` (existing behaviour). For any past year the window ends at `ZonedDateTime.of(year+1, 1, 1, …, userZone).toInstant()` — i.e. the last instant of that calendar year in the user's timezone — so the chart always shows the most recent 12 weeks of the year displayed in the heatmap. The frontend passes `selectedYear` to `getWeeklyPattern` on mount and on every year-selector change. The subtitle now reads "avg per weekday, last 12 weeks of {year}" to make clear that (a) it is an average divided by 12, not a daily total, and (b) it reflects the year currently shown in the heatmap.
