...
- **Reliable date-input fill for E2E (US-019)** — Playwright's `.fill()` on `<input type="date">` sets the DOM value but does not reliably fire React's synthetic `onChange` in Chromium. `us019-search-filter-tasks.spec.ts` replaces the `.fill()` + fixed `waitForTimeout` pattern with an `evaluate` call that uses the native `HTMLInputElement.prototype.value` setter followed by explicit `input` + `change` DOM events, then waits for the actual `/api/tasks` network response via `waitForResponse` before asserting the narrowed count.
